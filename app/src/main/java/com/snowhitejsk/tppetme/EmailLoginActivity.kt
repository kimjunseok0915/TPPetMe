...
import com.snowhitejsk.tppetme.databinding.ActivityEmailLoginBinding
...
class EmailLoginActivity : AppCompatActivity() {
    ...
    private fun clickSignIn(){
        ...
        //빈칸 채워졌는지 확인
        if(  email== ""){
            AlertDialog.Builder(this).setMessage("이메일을 입력하지 않았습니다. \n 이메일을 입력해 주세요").show()
            return
        }else if (password==""){
            AlertDialog.Builder(this).setMessage("비밀번호를 입력하지 않았습니다. \n 비밀번호를 입력해 주세요").show()
        }
        ...
        db.collection("emailUsers")
            ...
            .get().addOnSuccessListener {
                if(it.documents.size>0){  //where 조건에 맞는 데이터가 있다는 것임.
                    //로그인 성공
                    // [회원정보를 다른 Activity 에서도 사용할 가능선 있으므로.. 전역변수처럼 클래스이름만으로 사용가능한 변수에 저장하기]
